...
try: 
    ...
    rospy.loginfo('No PCA9685 Driver Detected. Publishing only!')
...
def compute_servo_commands(msg_in): 
    ...
    cmd00 = f_interp_rad_to_us_01_params(alpha0)
    cmd01 = f_interp_rad_to_us_12_params(beta1)
    cmd02 = f_interp_rad_to_us_23_params(beta2)
    cmd03 = f_interp_rad_to_us_34_params(gamma3)
    cmd04 = f_interp_rad_to_us_45_params(beta4)
    cmd05 = f_interp_rad_to_us_56_params(gamma5)

    cmd_all = [cmd00,cmd01,cmd02,cmd03,cmd04,cmd05]
...
def move_servos_and_set_joint_state(msg_in):
    ...
    if arm_is_present:
        command_servo(0,np.int(cmd00))
        command_servo(1,np.int(cmd01))    
        command_servo(2,np.int(cmd02))    
        command_servo(3,np.int(cmd03))    
        command_servo(4,np.int(cmd04))    
        command_servo(5,np.int(cmd05))    
